...
if [ "$LLM_NAME" = "220m" ]; then
    ...
    LLM_CHECKPOINT="/lustre/fs8/portfolios/llmservice/users/zhehuaic/pretrained/megatronnmt_any_en_500m.nemo"
    ...
    LLM_CHECKPOINT="/lustre/fs8/portfolios/llmservice/users/zhehuaic/pretrained/g_flant5_l.nemo"
...
VAL_CHECK_INTERVAL=2500 # If is an int n > 1, will run val every n training steps, if a float 0.0 - 1.0 will run val every epoch fraction, e.g. 0.25 will run val every quarter epoch
MAX_STEPS=500000
...
# model
PRETRAINED_AUDIO_MODEL="/lustre/fs8/portfolios/llmservice/users/zhehuaic/pretrained//stt_multilingual_fastconformer_hybrid_large_pc_blend_eu.nemo"
PRETRAINED_S2TT_MODEL="\"/workspace/model/megatron_audio_gpt_peft_tuning--validation_bleu=53.280-step=87504-epoch=4.ckpt\""
FREEZE_LLM=False
...
EXP_NAME=${MODEL_PREFIX}${CLUSTER}_${MODEL_NAME}_${LLM_NAME}_${DATASET}_lr${LR}wd${WEIGHT_DECAY}_${LR_SCHEDULER}_warmup${WARMUP_STEPS}_minlr${MIN_LR}_gbs${GLOBAL_BATCH_SIZE}_mbs${MICRO_BATCH_SIZE}_ep${MAX_EPOCHS}
...
CONFIG_NAME=modular_audio_t5_s2st_config
...
HYDRA_FULL_ERROR=1 TORCH_CUDNN_V8_API_ENABLED=1 python ${SCRIPT_PATH} \
    ...
    ++exp_manager.exp_dir=/results/s2st_interactive \
...
++model.data.validation_ds.output_dir=/results/s2st_interactive/ \
